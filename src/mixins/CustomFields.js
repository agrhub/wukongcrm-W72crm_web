        ...
        filedValidatesAPI(validatesParams).then(res => {
          ...
          if (resData.status === 1) {
            ...
            if (resData.ownerUserName) {
              reject(`${field.name} Đã tồn tại, người phụ trách là “${resData.ownerUserName}”`)
            } else if (resData.poolNames && resData.poolNames.length > 0) {
              reject(`${field.name} Đã tồn tại, hiện đang ở trong mục “${resData.poolNames.join('，')}”`)
            } else {
              reject(`${field.name} Đã tồn tại`)
            }
          }
        }).catch(() => {
          reject()
