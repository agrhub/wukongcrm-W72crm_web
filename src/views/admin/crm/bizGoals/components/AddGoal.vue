<template>
  <el-dialog
    ...
    title="Thiết lập mục tiêu kinh doanh"
    width="800px"
    ...
    <div v-loading="loading" class="add-goal">
      <flexbox class="select-wrapper">
        <flexbox-item class="select-item">
          <flexbox>
            ...
            <xh-user-cell
              ...
              placeholder="Chọn nhân sự hoặc phòng ban"
        ...
        <flexbox-item class="select-item">
          <span class="select-label">Chỉ số đánh giá (KPI)</span>
          <el-select
            v-model="typeSelect"
            class="select-condition">
            <el-option
      ...
      <div class="add-goal__handle">
        ...
        <span class="handle-label">Mục tiêu kinh doanh cả năm là </span>
        <el-input
          v-model="totalGoal"
          class="total-input"
          type="number"
          @input="inputChange('total')" />
        <span class="handle-label">đồng</span>
        <el-button type="primary" @click="averageClick">Phân bổ đều cho mỗi tháng</el-button>
      </div>
    ...
    <span
      slot="footer"
      class="dialog-footer">
      <el-button @click="closeClick">Đóng</el-button>
      <el-button
        type="primary"
...
export default {
  ...
  computed: {
    rangeLabel() {
      return this.type === 'user' ? 'Người đánh giá' : 'Bộ phận đánh giá'
    }
  },
  watch: {
    visible(value) {
  ...
  methods: {
    ...
    sureClick() {
      if (!this.selectDepOrUser.length) {
        this.$message.error(`Vui lòng chọn ${this.type == 'user' ? 'Người đánh giá' : 'Bộ phận đánh giá'}`)
      } else {
        const params = {
    ...
    getSetLabe(index, type) {
      return [['Tháng 1', 'Tháng 2', 'Tháng 3'],
        ...
        ['Tháng 10', 'Tháng 11', 'Tháng 12']][index][type]
    },
