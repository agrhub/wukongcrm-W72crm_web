<template>
  <div v-if="data && data.length === 0" style="color: #666;">Người đánh giá trống, đánh giá sẽ tự động thông qua</div>
  <el-timeline v-else>
    <el-timeline-item
          ...
          <flexbox
            ...
            <el-popover
              ...
              <div
                ...
                <div class="user__name">{{ flow.userList.length }} người {{ getWayTypeName(flow.type) }}</div>
                <div class="user__step">Cấp đầu tiên</div>
              </div>
            </el-popover>
          </flexbox>
        </template>
...
export default {
  ...
  filters: {
    step: function(index) {
      return 'Trước ' + Nzhcn.encodeS(index) + ' sau'
    },
