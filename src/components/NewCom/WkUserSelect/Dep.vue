      ...
      <div class="wk-user-select__header">
        ...
        >
          <template slot-scope="{ item }">
            ...
            <span style="color: #999;">{{ `(${item.post || 'Không có dữ liệu'})` }}</span>
          </template>
        </el-autocomplete>
      </div>
          ...
          <flexbox
            ...
            <template v-if="item.isDep">
              ...
              <el-button
                ...
                @click="lowerLevelClick(item)">Cấp dưới</el-button>
            </template>
            <template v-else>
              <xr-avatar
              ...
              <div class="user-info">
                ...
                <div class="user-post">{{ item.post || 'Không có dữ liệu' }}</div>
              </div>
            </template>
          </flexbox>
        </div>
  ...
  methods: {
    ...
    getDepUserList(deptId, depInfo) {
      ...
      request(deptId).then(res => {
        ...
        if (deptId == 0) {
          this.breadcrumbList = [{ label: 'Tất cả', deptList: deptList, employeeList: employeeList }]
        } else {
