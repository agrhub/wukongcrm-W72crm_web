<template>
  <div class="xh-user">
    ...
    <div class="xh-user__ft">
      <span class="select-info">Đã chọn <span class="select-info--num">{{ value ? value.length : 0 }}</span> mục</span>
      <el-button type="text" @click="clearAll">Xóa tất cả</el-button>
    </div>
  </div>
</template>
  ...
  computed: {
    showOptions() {
      return this.options.filter(item => {
        return (this.searchInput == '' ? true : PinyinMatch.match(item[this.props.label], this.searchInput))
      })
    }
  },
  watch: {
