  ...
  <div class="employee-department">
        ...
        <el-tab-pane
          ...
          <div class="search-list">
            <el-checkbox
              ...
              v-model="checkAll"
              class="all-check"
              @change="handleCheckAllChange">Chọn tất cả</el-checkbox>
    ...
    <div
      ...
      <div class="checked-top">
        <span class="select-info">Đã chọn <span class="select-info--num">{{ userSelectCount + depSelectCount }}</span> mục</span>
        <el-button
          type="text"
          @click="clearAll">Xóa hết</el-button>
      </div>
  ...
  methods: {
    ...
    userSearchChange(val) {
      this.userList = this.userList.map(item => {
        item.hidden = (val == '' ? false : !PinyinMatch.match(item.realname, val))
        return item
      })
    },
    depSearchChange(val) {
      this.depShowList = this.depShowList.map(item => {
        item.hidden = (val == '' ? false : !PinyinMatch.match(item.name, val))
        return item
      })
    },
    /**
