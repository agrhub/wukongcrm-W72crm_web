...
export default {
  ...
  created() {
    for (let index = 1; index <= 20; index++) {
      ...
      if (index === 1) {
        this.sendLevelObj[index] = 'Cấp trên trực tiếp'
        this.topLevelObj[index] = 'Duy nhất'
      } else {
    ...
    getContent() {
      ...
      if (examineType === 1) {
        if (userList.length === 1) {
          return `${examineTypeName} 1 người`
        } else if (userList.length > 1) {
          return `${userList.length} người ${wayTypeObj[type]}`
        } else {
          return `Vui lòng chọn một người phê duyệt`
        }
        ...
        if (roleId) {
          return `Người đề xuất ${roleObj.roleName} tùy chọn`
        } else {
        ...
        if (type === 1) {
          if (!roleId) {
            ...
            if (overType === 0) {
              return `Từ cấp trên trực tiếp đến ${roleObj.roleName}`
            } else if (overType === 1) {
              return `Từ cấp trên trực tiếp đến người để xuất ${this.levelObj[parentLevel]}`
            } else {
              return ''
            }
          }
        } else {
          return `Từ cấp trên trực tiếp đến cấp trên trong cơ cấu tổ chức ${this.topLevelObj[tempParentLevel]}`
        }
      }
