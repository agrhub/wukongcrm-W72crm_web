  ...
  <div
    ...
    <div
      ...
      <div
        v-if="!bigShowType.isImage"
        class="file-show">
        <div v-if="!showPreviewBtn" class="title">Định dạng tệp đính kèm không hỗ trợ xem trước, vui lòng tải xuống và xem</div>
        <i
          class="el-icon-close"
          @click="closeViewer" />
        <div class="content">
          ...
          <div class="file-handle">
            <el-button
              ...
              type="primary"
              plain
              @click.native="previewFile">Xem trước</el-button>
            <el-button
              type="primary"
              @click.native="downloadFile">Tải về</el-button>
          </div>
        </div>
      </div>
      <!-- tips -->
...
export default {
  ...
  methods: {
    ...
    handlePrev() {
      if (this.imgIndex <= 0) {
        this.tips('Chỉ có một File duy nhất!')
        this.imgIndex = 0
      } else {
        if (this.$refs.bigImg) {
    ...
    handleNext() {
      if (this.imgIndex + 1 >= this.imgData.length) {
        this.tips('Đây là File cuối cùng!')
      } else {
        if (this.$refs.bigImg) {
          this.$refs.bigImg.style.transform = 'rotate(0deg)'
          this.rotateDeg = 0
