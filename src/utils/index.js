...
export function verifyFileTypeWithFileName(fileName, type = 'excel', messageShow = true) {
  ...
  if (!pass && messageShow) {
    Message({
      message: 'Vui lòng chọn đúng loại tệp',
      type: 'error'
    })
  }
  return pass
...
export function formatTime(time) {
  ...
  if (timeYear == nowYear) {
    return timeMoment.format('ngày DD tháng MM')
  } else {
    return timeMoment.format('ngày DD tháng MM năm YY')
  }
}
...
export function downloadFile(data) {
  downloadFileAPI(data.path).then(res => {
    ...
    downloadFileWithBuffer(blob, data.name || 'tập tin')
  }).catch(() => {})
}
...
export function getExcelLines(file) {
  ...
  reader.onload = function(evt) {
    ...
    try {
      ...
      console.log('Loại tệp không chính xác', e)
      return
    }
  }
}
