...
const confirmMessage = debounce(1000, (message) => {
  MessageBox.confirm(message, 'Thông báo', {
    ...
    if (response.status === 200 && response.config.responseType === 'blob') { // 文件类型特殊处理
      ...
      if (res.code === 302) {
        if (res.data && res.data.extra === 1) {
          confirmMessage(`Tài khoản của bạn ${res.data.extraTime} đã được đăng nhập ở một máy khác. Nếu bạn không thực hiện việc này, mật khẩu có thể đã bị lộ. Bạn nên thay đổi mật khẩu`)
        } else {
          clearCacheEnterLogin()
        }
  ...
  error => {
    if (error.response) {
      ...
      if (response.status == 500) {
        errorMessage('Lỗi kết nối mạng, vui lòng kiểm tra mạng internet của bạn')
